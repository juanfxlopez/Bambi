...
def create_watershed(labels):
    mask = labels.copy()
    mask[mask > 0] = 1
    dilated = binary_dilation(mask, iterations=50)
...
for n, id_ in tqdm(enumerate(train_ids), total=len(train_ids)):
    ...
    for mask_file in next(os.walk(path+"/masks/"))[2]:
        ...
        mask_ = cv2.imread(path + "/masks/" + mask_file, cv2.IMREAD_UNCHANGED)
        mask_ = cv2.resize(mask_,(512,512))
        # Find contours at a constant value of 0.8
        mask_plt_ = mask_.copy()
        mask_plt=mask.copy()
        ...
        #bi dilated = binary_dilation(mask, iterations=50)
        kernel = np.ones((3,3), np.uint8)
        mask_plt_ =  cv2.erode(mask_plt_, kernel, iterations=1)
        mask_plt = cv2.erode(mask_plt, kernel, iterations=1)
        #cv2.drawContours(mask_plt_,contours_, -1, (255,0,0), 2)
        #cv2.drawContours(mask_plt,contours, -1, (255,0,0), 2)

        #cv2.imshow('mask_plt', mask_plt)
        #cv2.imshow('mask2_plt', mask2_plt)


        intermask_= cv2.bitwise_and(mask_plt_,mask_plt)
    ...
    for count,value in enumerate(contours2):
            ...
            cv2.drawContours(mask3_plt, [value], 0, random_color, 2)
    ...
    #mask_contourscolor.save("../data/GenData/TrainData/contourscolor/" + str("%04d" % (n + 65)) + "_.png")
